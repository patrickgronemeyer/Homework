...
// Filter to get only user 5's posts
const user5Posts = posts.filter(post => post.userId === 5);  // posts.filter(post => post.userId === 5) isolates the posts where the userId is 5

// Create a list of title and body pairs
const titleBodyPairs = user5Posts.map(post => {
  return { title: post.title, body: post.body };
});
// .map(post => { return { title: post.title, body: post.body }; }) creates a new array containing objects with title and body from each post.

// Displaying the results
console.log("List of User 5's Posts:", titleBodyPairs);
